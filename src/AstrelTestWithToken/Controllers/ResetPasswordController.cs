...
namespace AstrelTestApi.Controllers
{
    ...
    {
        private readonly AstralTest.Domain.Interfaces.IAuthorizationService _authorizationService;

        public ResetPasswordController(AstralTest.Domain.Interfaces.IAuthorizationService authorizationService)
        {
        ...
        [HttpPost("SendSms")]
        public async Task<Guid> SendCodeToReset([FromQuery] string userName)
        {
            var resultGuid = await _authorizationService.SendSmsToResetPassword(userName);
            return resultGuid;
        }
